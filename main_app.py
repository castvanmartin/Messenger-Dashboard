...
with header:
    st.title("Welcome to my Messenger inbox data analysis project")
    st.markdown("How to denerate dashboard?")
    ...
    st.text("To generate dashboard upload output_zip file generated by prepare_zip.py script.")
...
if uploaded_file is not None:
    ...
    with dataset:
        ...
        #msg_counts = pd.DataFrame(df["Chat"].value_counts()).sort_values(by=["Chat"], ascending=False)
        #figure = px.bar(msg_counts, x=msg_counts.index, y="Chat")
